    ...
    internal fun retrieveDataForObject(sportsObject: SportsEvent): MutableList<BetsAdapter.MyGroupItem> {
        ...
        return mutableListOf<BetsAdapter.MyGroupItem>().apply {
            this.add(BetsAdapter.MyGroupItem(0, "Big Win Little Win",sportsObject).also {
                (it.children as MutableList).add(BetsAdapter.MyChildItem(0, "Home 1-39", 3.00))
                (it.children).add(BetsAdapter.MyChildItem(1, "Home 40+", 13.00))
                (it.children).add(BetsAdapter.MyChildItem(2, "Draw", 56.00))
                (it.children).add(BetsAdapter.MyChildItem(3, "Away 1-39", 2.10))
                (it.children).add(BetsAdapter.MyChildItem(4, "Away 40+", 4.50))
            })

            this.add(BetsAdapter.MyGroupItem(1, "Time First Goal Kicked",sportsObject).also {
                (it.children as MutableList).add(BetsAdapter.MyChildItem(0, "0:00 - 1:00", 7.00))
                (it.children).add(BetsAdapter.MyChildItem(1, "1:01 - 2:00", 4.50))
                (it.children).add(BetsAdapter.MyChildItem(2, "2:01 - 3:00", 5.50))
                (it.children).add(BetsAdapter.MyChildItem(3, "3:01 - 4:00", 6.50))
                (it.children).add(BetsAdapter.MyChildItem(4, "4:01 - 5:00", 8.50))
                (it.children).add(BetsAdapter.MyChildItem(5, "5:01+", 2.80))
            })
            ...
            for (i in 2..19) {
                ...
                for (j in 0..4) {
                    ...
                    group.children.add(BetsAdapter.MyChildItem(j.toLong(), "child $j", 0.00))
                }
                this.add(group)
            }
